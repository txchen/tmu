...
TMU will publish its own source as a prebuilt ESM bundle while keeping Node built-ins and declared npm runtime dependencies external. The package supports `npx @txchen/tmu` and global installation as `@txchen/tmu`, while retaining `tmu` as the executable name. It contains build output and user documentation rather than TypeScript source, and requires no TypeScript loader or compilation on the user's machine. npm and `package-lock.json` replace Bun throughout the repository; tsdown builds the package, Vitest runs tests, and node-pty provides development-only real-terminal testing.
