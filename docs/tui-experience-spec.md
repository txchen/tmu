...
In Results, Enter invokes Play Now, `a` adds a healthy Track to the Queue, `N` moves/adds it to Play Next, `e` opens Rename Track for a healthy Track, `d` opens confirmed permanent deletion for a healthy Track or confirmed cleanup for an incomplete entry, and `/` focuses Search. Search filters live while typing; Enter or Esc transfers focus to Results without clearing the query, with Enter selecting the first match but never starting playback from the Search field. Playback, Queue, and rename actions are visibly disabled and omitted or dimmed in contextual help when the selected row is incomplete.
