...
TMU is a keyboard-first YouTube Cache player with three top-level tabs: Playback, Library, and YouTube Downloader. It always opens on Playback and restores the Last Playlist Snapshot without autoplay.
...
Uppercase `P` opens the Playlist Manager outside text entry. It selects the Active Playlist and supports switching with `Enter`, creating with `c`, renaming with `e`, deleting with `x`, and persistent reordering with `J`/`K`; lowercase `p` remains Previous Track. Switching stops playback without autoplay and restores the destination Playlist's Current Track, saved position, stopped-or-resumable state, and Repeat All setting.
